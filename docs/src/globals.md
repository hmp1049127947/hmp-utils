[**hmp-utils**](README.md) • **Docs**

***

# hmp-utils

## url工具

- [getUrlParams](functions/getUrlParams.md)
- [getUrlParamsByKey](functions/getUrlParamsByKey.md)
- [obj2UrlString](functions/obj2UrlString.md)

## 元素

- [scrollToTop](functions/scrollToTop.md)
- [scrollToView](functions/scrollToView.md)

## 加解密

- [AesCrypto](classes/AesCrypto.md)
- [base64Decode](functions/base64Decode.md)
- [base64Encode](functions/base64Encode.md)
- [md5Encrypt](functions/md5Encrypt.md)

## 异步

- [parallel](functions/parallel.md)
- [retry](functions/retry.md)
- [sleep](functions/sleep.md)
- [tryIt](functions/tryIt.md)

## 类型判断

- [isClient](variables/isClient.md)
- [isInIframe](variables/isInIframe.md)
- [isServer](variables/isServer.md)
- [is](functions/is.md)
- [isArray](functions/isArray.md)
- [isBoolean](functions/isBoolean.md)
- [isCarNo](functions/isCarNo.md)
- [isDate](functions/isDate.md)
- [isDefined](functions/isDefined.md)
- [isElement](functions/isElement.md)
- [isEmail](functions/isEmail.md)
- [isEmpty](functions/isEmpty.md)
- [isFunction](functions/isFunction.md)
- [isIdCard](functions/isIdCard.md)
- [isImg](functions/isImg.md)
- [isJSON](functions/isJSON.md)
- [isMap](functions/isMap.md)
- [isMobile](functions/isMobile.md)
- [isNil](functions/isNil.md)
- [isNull](functions/isNull.md)
- [isNullAndUnDef](functions/isNullAndUnDef.md)
- [isNumber](functions/isNumber.md)
- [isObject](functions/isObject.md)
- [isPromise](functions/isPromise.md)
- [isRegExp](functions/isRegExp.md)
- [isString](functions/isString.md)
- [isUnDefined](functions/isUnDefined.md)
- [isUrl](functions/isUrl.md)
- [isWindow](functions/isWindow.md)

## 缓存工具

- [LocalStore](classes/LocalStore.md)
- [localStore](variables/localStore.md)

## 自定义指令

- [directive](variables/directive.md)
- [clickOutsideDirective](functions/clickOutsideDirective.md)
- [dragDirective](functions/dragDirective.md)
- [lazyDirective](functions/lazyDirective.md)

## 随机工具

- [getUUID](functions/getUUID.md)
- [random](functions/random.md)
- [randomArray](functions/randomArray.md)
